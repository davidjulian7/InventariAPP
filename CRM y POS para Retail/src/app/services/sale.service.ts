...
export class SaleService {
  ...
  static async getRecent(storeId: number, limit = 5): Promise<Sale[]> {
    const ventas = localDb.getAll<any>('venta')
  ...
  static async create(cart: { id: number; codigo: string; nombre: string; precio: number; qty: number }[], metodoPago: string, storeId: number, userId: number): Promise<Sale> {
    ...
    for (const item of cart) {
      ...
      const invs = localDb.query<any>('inventario', (i: any) => i.producto_id === item.id && i.tienda_id === storeId)
      if (invs.length > 0) {
        localDb.update('inventario', invs[0].id, {
  ...
  private static withItems(venta: any): Sale {
    const detalles = localDb.query<any>('venta_detalle', (d: any) => d.venta_id === venta.id)
